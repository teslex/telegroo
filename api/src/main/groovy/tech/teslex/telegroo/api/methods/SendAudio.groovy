...
trait SendAudio {
	...
	def sendAudio(File audio, Map params = [:]) {
		sendAudio(audio, lastUpdate.updateData[lastUpdate.updateType].chat.id, params)
	}
	...
	def sendAudioId(String audioId, Map params = [:]) {
		sendAudioId(audioId, lastUpdate.updateData[lastUpdate.updateType].chat.id, params)
	}
